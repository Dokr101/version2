...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book_room'])) {
    ...
    if ($existing_booking) {
        ...
        // Create booking
        $stmt = $pdo->prepare("
            INSERT INTO bookings (user_id, room_id, checkin, checkout, total_price) 
            VALUES (?, ?, ?, ?, ?)
        ");
        if ($stmt->execute([$_SESSION['user_id'], $room_id, $checkin, $checkout, $total_price])) {
            $_SESSION['success'] = "Room booked successfully! Please complete the payment in your dashboard.";
